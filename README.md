...
3. Do one of there:
   - **Recommended**: See `Example_code.ipynb` on how to use this tool as a **package**. Note: ensure `network_data_dir = 'network_data/'` in `configFile.py`.
   - **Alternative**: Run `runSimplification.py` in an IDE (or otherwise). Note: set `network_data_dir = '../../network_data/'` in `configFile.py`.
